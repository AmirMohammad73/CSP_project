    ...
    methods: {
      async handleRowClick(item) {
        if (this.currentLevel === 'کشور') {
          ...
          this.breadcrumb.push({ text: item.locname, disabled: false });
          this.currentLevel = 'shahrtitle';
          this.selectedshahrestan = item.locname;
          await this.fetchShahrData(this.selectedostan, item.locname);
        }
      },
      ...
      navigateBreadcrumb(item) {
        if (item.text === 'کشور') {
          ...
          this.isRole4 ? this.fetchShahrData(this.selectedostan, this.selectedshahrestan) : this.fetchShahrestanData(this.selectedostan);
