...
app.get('/api/locations/shahr', authenticateToken, async (req, res) => {
  const { ostantitle, shahrestantitle } = req.query;
  try {
    const data = await getShahrData(ostantitle, shahrestantitle);
    res.json(data);
  } catch (err) {
    console.error('API error:', err);
    res.status(500).json({ error: 'Internal server error' });
...
app.post('/getdadehamaeiusers', authenticateToken, async (req, res) => {
    try {
        const { role, permission } = req.user;
        const data = await getDadehAmaeiUsers(role, permission);
        res.json(data);
    } catch (err) {
        console.error('API error:', err);
        res.status(500).json({ error: 'Internal server error' });
